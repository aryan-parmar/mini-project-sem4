...
<body>
    <section class="home">
        ...
        <section class="add_post_area">
            ...
            <?php
            if (!isset($_GET['username'])) {
                $query = "SELECT * FROM follow WHERE pending=0 AND fk_other_user_id = " . $_SESSION['user_id'];
            }else{
                $query = "SELECT * FROM user_data WHERE username = '" . $_GET['username']."'";
                $result = mysqli_query($db, $query);
                ...
                $query = "SELECT * FROM follow WHERE pending=0 AND fk_other_user_id = " . $res['user_id'];
            }
            $result = mysqli_query($db, $query);
            ...
            while($row = mysqli_fetch_assoc($result)){
                ...
                <button class="follow" data-id="' . $user["user_id"] . '" >Remove</button>
                </div>';
